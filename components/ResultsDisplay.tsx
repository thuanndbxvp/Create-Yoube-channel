...
const IdeaCard: React.FC<{ idea: ChannelIdeaSet; index: number }> = ({ idea, index }) => {
  ...
  return (
    <div className="bg-slate-900/50 rounded-lg p-6 border border-slate-700 transform transition-transform hover:scale-[1.02] hover:border-sky-500/50">
      <h3 className="text-xl font-bold text-sky-400 mb-4">Ý tưởng {index + 1}: <span className="text-white">{idea.channelName}</span></h3>
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {idea.hashtags.map((tag, i) => <span key={i} className="bg-slate-700 text-sky-300 text-sm font-medium px-2.5 py-1 rounded-full">{tag}</span>)}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-4 border-t border-slate-700">
          <div>
            ...
            <div className="relative">
              ...
              <button onClick={() => copyToClipboard(idea.bannerPrompt, 'banner')} className="absolute top-2 right-2 p-1.5 bg-slate-600 hover:bg-slate-500 rounded-md text-slate-200 transition-colors text-xs">
                {copied === 'banner' ? 'Đã chép!' : 'Chép Prompt'}
              </button>
            </div>
          </div>
          <div>
            ...
            <p className="text-slate-400 text-sm italic mb-2">{idea.logoIdea}</p>
             <div className="relative">
              <pre className="bg-slate-800 p-3 rounded-md text-sm text-slate-300 overflow-x-auto font-mono"><code>{idea.logoPrompt}</code></pre>
               <button onClick={() => copyToClipboard(idea.logoPrompt, 'logo')} className="absolute top-2 right-2 p-1.5 bg-slate-600 hover:bg-slate-500 rounded-md text-slate-200 transition-colors text-xs">
                {copied === 'logo' ? 'Đã chép!' : 'Chép Prompt'}
              </button>
            </div>
          </div>
        </div>
